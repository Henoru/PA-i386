					 ...
					 SDL_Surface *dst, SDL_Rect *dstrect)
{
	...
	int16_t sx = (srcrect == NULL ? 0 : srcrect->x);
	int16_t sy = (srcrect == NULL ? 0 : srcrect->y);
	int16_t dx = (dstrect == NULL ? 0 : dstrect->x);
	int16_t dy = (dstrect == NULL ? 0 : dstrect->y);
	uint16_t w = (srcrect == NULL ? src->w : srcrect->w);
	uint16_t h = (srcrect == NULL ? src->h : srcrect->h);
	 ...
	 * `dst' surface.
	 */
	uint8_t plen=src->format->BytesPerPixel;
	assert(plen==dst->format->BytesPerPixel);
	uint8_t* srcpos=src->pixels+(src->w*sy*plen)+(sx*plen);
	uint8_t* dstpos=dst->pixels+(dst->w*dy*plen)+(dx*plen);
	for(uint16_t i=0;i<h;i++){
		memcpy(dstpos+i*dst->w*plen,srcpos+i*src->w*plen,w*plen);
	}
...
void SDL_FillRect(SDL_Surface *dst, SDL_Rect *dstrect, uint32_t color)
{
	...
	int16_t dx = (dstrect == NULL ? 0 : dstrect->x);
	int16_t dy = (dstrect == NULL ? 0 : dstrect->y);
	uint16_t w = (dstrect == NULL ? dst->w : dstrect->w);
	uint16_t h = (dstrect == NULL ? dst->h : dstrect->h);
	if(dst->w - dx < w) { w = dst->w - dx; }
	if(dst->h - dy < h) { h = dst->h - dy; }
	if(dstrect != NULL) {
		dstrect->w = w;
	...
	uint8_t plen=dst->format->BytesPerPixel;
	uint8_t* dstpos=dst->pixels+(dst->w*dy*plen)+(dx*plen);
	for(uint16_t i=0;i<h;i++){
		memset(dstpos+(dst->w*i*plen),(uint8_t)color,w*plen);
	}
}
